module.exports = async function(Stable_Version) {
    ...
    //make request https://raw.githubusercontent.com/MDong-06/fca-donq-dev/main/package.json
    const { body } = await got('https://raw.githubusercontent.com/MDong-06/fca-donq-dev/main/package.json');
    ...
    const LocalVersion = require('../../package.json').version;
        if (Number(LocalVersion.replace(/\./g,"")) < Number(json.version.replace(/\./g,"")) && global.Fca.Require.FastConfig.Stable_Version.Accept == false || Stable_Version && Number(LocalVersion.replace(/\./g,"")) != Number(Stable_Version.replace(/\./g,""))) {
            ...
            try {
                execSync(`npm install fca-donq-dev@${Version}`, { stdio: 'inherit' });
                ...
                try {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    execSync(`npm install fca-donq-dev@${Version} --force`, { stdio: 'inherit' });
                    log.info("[ FCA-UPDATE ] •","Update Complete, Restarting...");
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    Database().set("Instant_Update", Date.now());
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    ...
                    try {
                        ...
                        fs.rmdirSync((process.cwd() + "/node_modules/fca-donq-dev" || __dirname + '../../../fca-donq-dev'), { recursive: true });
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        execSync(`npm install fca-donq-dev@${Version}`, { stdio: 'inherit' });
                        log.info("[ FCA-UPDATE ] •","Update Complete, Restarting...");
                        await new Promise(resolve => setTimeout(resolve, 3000));
